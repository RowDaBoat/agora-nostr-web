import { a as attr_class, b as attr, s as stringify, f as bind_props, h as spread_props, p as props_id, k as attributes, j as derived, e as ensure_array_like } from './index2-DpBdzO5t.js';
import { s as settings } from './ndk.svelte-BfhDBrJw.js';
...
import { d as createId, b as boxWith, m as mergeProps, e as attachRef, c as createBitsAttrs } from './scroll-lock-YhRhLzPR.js';
...
function Icon($$renderer, $$props) {
  ...
  const isFilled = name === "heart-filled" || name === "bookmark-filled";
  $$renderer.push(`<svg${attr_class(`${stringify(sizeClasses[size])} ${stringify(className)}`)}${attr("fill", isFilled ? "currentColor" : "none")} stroke="currentColor" viewBox="0 0 24 24"${attr("aria-label", ariaLabel)}${attr("role", ariaLabel ? "img" : void 0)}>`);
  if (name === "spinner") {
    $$renderer.push("<!--[-->");
    $$renderer.push(`<path fill="currentColor"${attr("d", path)}></path>`);
  } else {
    $$renderer.push("<!--[!-->");
    $$renderer.push(`<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"${attr("d", path)}></path>`);
  }
  $$renderer.push(`<!--]--></svg>`);
}
...
function RelayPublishDropdownContent($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let {
      selectedRelayUrls,
      isProtected = false,
    ...
    if (showProtected) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="px-2 py-2 mb-2 border-b border-border"><div class="flex items-center justify-between"><div class="flex items-center gap-2"><svg${attr_class(`w-4 h-4 ${stringify(isProtected ? "text-primary" : "text-muted-foreground")}`)} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg> `);
      Label($$renderer2, {
        for: "protected-switch",
        class: `text-sm ${stringify(isProtected ? "text-primary font-medium" : "text-muted-foreground")}`,
        children: ($$renderer3) => {
      ...
      $$renderer2.push(`<!--]--></div></div> <button type="button" role="switch"${attr("aria-checked", isProtected)} id="protected-switch"${attr_class(`relative inline-flex h-[1.15rem] w-8 shrink-0 items-center rounded-full border border-transparent transition-colors focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 ${stringify(isProtected ? "bg-primary" : "bg-input")}`)}><span${attr_class(`pointer-events-none block size-4 rounded-full bg-background ring-0 transition-transform ${stringify(isProtected ? "translate-x-[calc(100%-2px)]" : "translate-x-0")}`)}></span></button></div></div>`);
    } else {
      $$renderer2.push("<!--[!-->");
    }
    $$renderer2.push(`<!--]--> <div class="text-xs text-muted-foreground px-2 py-1.5 font-medium">Select Relays to Publish</div> <!--[-->`);
    const each_array = ensure_array_like(allRelays);
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      $$renderer2.push(`<div class="group relative flex items-center overflow-hidden">`);
      Button($$renderer2, {
        variant: "ghost",
        onclick: () => onToggleRelay(relay.url),
        class: `flex-1 justify-start h-auto py-2 min-w-0 pr-16 ${stringify(isSelected ? "bg-muted/50" : "")} ${stringify(isReadOnly ? "opacity-60" : "")}`,
        children: ($$renderer3) => {
          RelayIcon($$renderer3, { relayUrl: relay.url, size: "md", class: "mr-3" });
          $$renderer3.push(`<!----> <div class="flex-1 min-w-0 text-left overflow-hidden"><div class="flex items-center gap-2 min-w-0"><div class="text-sm font-medium text-foreground truncate flex-1 min-w-0">${escape_html(relayInfo.info?.name || relay.url.replace("wss://", "").replace("ws://", ""))}</div> `);
          if (isReadOnly) {
            $$renderer3.push("<!--[-->");
            $$renderer3.push(`<span class="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded whitespace-nowrap flex-shrink-0">read-only</span>`);
          } else {
            $$renderer3.push("<!--[!-->");
          }
          $$renderer3.push(`<!--]--></div> `);
          if (relayInfo.info?.description) {
            $$renderer3.push("<!--[-->");
            $$renderer3.push(`<div class="text-xs text-muted-foreground truncate overflow-hidden">${escape_html(relayInfo.info.description)}</div>`);
          } else {
            $$renderer3.push("<!--[!-->");
          }
          $$renderer3.push(`<!--]--></div> `);
          if (isSelected) {
            $$renderer3.push("<!--[-->");
            $$renderer3.push(`<svg class="w-5 h-5 text-primary flex-shrink-0 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>`);
          } else {
            $$renderer3.push("<!--[!-->");
          }
          $$renderer3.push(`<!--]-->`);
    ...
    $$renderer2.push(`<!--]--></div>`);
    bind_props($$props, { isProtected });
  });
}

export { Icon as I, Label as L, RelayPublishDropdownContent as R };
//# sourceMappingURL=RelayPublishDropdownContent-CzmVdnh5.js.map
