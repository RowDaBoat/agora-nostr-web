...
<script lang="ts">
  ...
  let user = $state(ndk.getUser({ pubkey: bech32 }));
  ...
  $effect(() => {
    ...
    const ndkUser = ndk.getUser({ pubkey: bech32 });
    user = ndkUser;
    ndkUser.fetchProfile().then(p => profile = p);
  });
...
{:else}
  <a
    ...
    onclick={handleClick}
  >
    <Avatar {user} size={16} class="mention-avatar" />
